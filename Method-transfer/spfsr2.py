...
class SpFSRKernel:
    ...
    def __init__(self, params):
        ...
        ### 修改的
        self.xtrain = params['xtrain']
        self.ytrain = params['ytrain']
        self.xvalid = params['xvalid']
        self.yvalid = params['yvalid']
    ...
    def eval_feature_set(self, cv_task, curr_imp):
        ...
        # 修改后
        xtrain = x_fs
        ytrain = self._output_y
        xvalid = self.xvalid
        yvalid = self.yvalid
        opts = self.opts
        opts['dim'] = np.size(xvalid, 1)
        X = np.ones((1, np.size(xtrain, 1)))
        xvalid_active = xvalid[:, self._idx_active]
        xvalid_fs = xvalid_active[:, selected_features]
        xvalid = xvalid_fs
        # 修改2
        # xtrain = self.xtrain
        # ytrain = self.ytrain
        ...
        # yvalid = self.yvalid
        # opts = self.opts
        # X = np.zeros((1, np.size(xtrain, 1)))
        # _idx_active = np.array(self._idx_active)
        # selected_features = np.array(selected_features)
        # X[:, _idx_active[selected_features[:]]] = 1
        fit = 1 - Fun(xtrain, xvalid, ytrain, yvalid, X, opts)
        best_value_mean = fit
        best_value_std = 0
        ##
        # scores = np.concatenate((scores, np.tile(fit, 12)))
    ...
    def run_kernel(self):
        ...
        while curr_iter_no < self._iter_max:
            ...
            self._iter_results['curve'].append(fs_perf_output[0])
            ...
            if curr_iter_no % self._print_freq == 0 and False:
...
class SpFSR:
    def run(self,
        ...
        sp_params['xtrain'] = xtrain
        sp_params['ytrain'] = ytrain
        sp_params['xvalid'] = xvalid
        sp_params['yvalid'] = yvalid
        sp_params['opts'] = opts
        ######################################
