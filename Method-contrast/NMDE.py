...
import operator
import geatpy as ea
from NMDE3 import improved_evaluation
from NMDE4 import subset_repairing_scheme
from NMDE5 import remove_empty
...
def fit_train(x1, data):
    ...
    if np.count_nonzero(x) == 0:
     ...
     X_New = np.zeros([1, np.size(x_train, 1)])
     X_New[0, value_position] = 1
     scores = Fun(xtrain, xvalid, ytrain, yvalid, X_New[0, 1:], opts)
     f1 = scores
     result['Gbin'].append(X_New[0, 1:])
     result['curve'].append(f1)
     ...
     # f1 = np.mean(1 - scores)
     f2 = (len(value_position)-1)/(x_train.shape[1] - 1)
...
def proposed_NGI(niche_offspring,ss,offspring,member,ii,f_mu,fit_num,Max_FES):
...
def first_nondominated(pop):
    ...
    [levels1, criLevel1] = ea.ndsortDED(PF, 1)
    x1 = 1 * (levels1 == 1.0)
    x1 = "".join(map(str, x1))
...
def main(xtrain, xvalid, ytrain, yvalid, opts):
    ...
    # x_train = np.concatenate((np.expand_dims(ytrain, axis=1), xtrain), axis=1)
    x_train = np.concatenate((np.expand_dims(y, axis=1), x), axis=1)
    data = {'all': x_train, 'xtrain': xtrain, 'xvalid': xvalid, 'ytrain': ytrain, 'yvalid': yvalid, 'opts': opts, 'result': result}
    ...
    toolbox.register("evaluate", fit_train, data=data)
    toolbox.register("select1", NMDE2.selNS)
    ...
    for gen in range(1, NGEN):
        ...
        for ii in range(MU):#####upate the whole population
            ...
            y_new, nbest = proposed_NGI(niche_offspring,ss_i,offspring,offspring[ii],ii,0.5,fit_num,Max_FES)
            pop_new[ii] = cxBinomial(offspring[ii],y_new,0.5)###crossover
            pop_new = remove_empty(pop_new)
            del pop_new[ii].fitness.values
        ##################################################
        ##IF one solution has shown before, then its fitness values will be taken from pop_u. This is to reduce the training time
        fit_improved, fit_num = improved_evaluation(pop_new,pop_u,fit_num,data)##<class 'numpy.ndarray'>
        ...
        pop1, fit_num,pop_unique = subset_repairing_scheme(pop1,pop_u,fit_num,data,ee)
    ...
    return offspring,first_rank_population,result
...
def fs(xtrain, xvalid, ytrain, yvalid, opts):
    ...
    # creator.create("Individual", array.array, typecode='d', fitness=creator.FitnessMin)
    # toolbox = base.Toolbox()
    pop,first_rank_population,result = main(xtrain, xvalid, ytrain, yvalid, opts)
    index_non33 = first_nondominated(pop)
    # front_non, front = new_selection.output2(pop,0.0001)
    ...
    NMDE_data = {'sf': Gbin, 'c': curve, 'nf': num_feat}
