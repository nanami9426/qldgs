...
def rating_information_entropy_calculation(fitness_vector, problem_type, t, record_active_window):
    ...
    try:
        ...
        min_num = grade_list[0]
        max_num = grade_list[-1]
    if min_num != lt:
...
def new_mechanism(X, fit, a, b, part_num):
    ...
    for top_individual in pop_new_flag:
        ...
        for feature_index_1, feature_selection_value_1 in enumerate(top_individual):
            if feature_selection_value_1 == 1: feature_share_list[feature_index_1] += 1
...
def fs(xtrain, xvalid, ytrain, yvalid, opts):
    ...
    for i in range(N):
        fit[i, 0] = 1-Fun(xtrain, xvalid, ytrain, yvalid, X[i, :], opts)
        if fit[i, 0] > fitG:
            Xgb[0, :] = X[i, :]
            fitG = fit[i, 0]
    ...
    with open('../result/fitness_each_generation', 'a') as f:  # 设置文件对象
        f.write('\n' + 'ECGA_10_fold_UCI_original_' + opts['classifer'] + ',')  # 将字符串写入文件中
        f.close()
    curve[0, t] = fitG.copy()
    ...
    with open('../result/fitness_each_generation', 'a') as f:  # 设置文件对象
        ...
        f.close()
    '''
    # print("Generation:", t + 1, "Best (GA):", curve[0, t])
    ...
    while t < max_iter:
        if t == 20:
            ...
            # print('种群整体等级熵{}\t适应值上下界活动窗口{}\t落入各个等级个体数{}\t{}个等级'.format(
            #     rating_based_entropy_value, active_window,
            #     n_grade_list, K))
        elif t > 20:
        ...
        for i in range(2 * Nc):
            Fnew[i, 0] = 1-Fun(xtrain, xvalid, ytrain, yvalid, Xnew[i, :], opts)
            if Fnew[i, 0] > fitG:
                Xgb[0, :] = Xnew[i, :]
                fitG = Fnew[i, 0]
                cnt_best = 0
        ...
        for i in range(N):
            ...
            if ind[i, 0] >= N:
                cnt_replace += 1
        # print(len(X), len(Xnew), '子代替换父代数{}\t最优停滞轮数{}'.format(cnt_replace // 2, cnt_best))
        meanFit[0, t - 1] = np.mean(fit)
        # with open('../result/fitness_each_generation', 'a') as f:  # 设置文件对象
        #     f.write(str(meanFit[0, t - 1]) + ',')  # 将字符串写入文件中
        #     f.close()
        if t > 20 and rand() > 0.5 and rating_based_entropy_value > 0.1:
            cnt_start_new += 1
            ...
            # print('1 random', feature_share_random_list, len(feature_share_random_list))
            if feature_share_choosed_list == []:
                feature_share_choosed_list, feature_share_abandoned_list, feature_share_random_list = new_mechanism(X,
                                                                                                                    fit,
                                                                                                                    a=0.7,
                                                                                                                    b=0.3,
                                                                                                                    part_num=3)
            ...
            if feature_share_abandoned_list == []:
                feature_share_choosed_list, feature_share_abandoned_list, feature_share_random_list = new_mechanism(X,
                                                                                                                    ...
                                                                                                                    b=0.1,
                                                                                                                    part_num=3)
                # print('3 choosed', feature_share_choosed_list, len(feature_share_choosed_list))
                # print('3 abandoned', feature_share_abandoned_list, len(feature_share_abandoned_list))
                # print('3 random', feature_share_random_list, len(feature_share_random_list))
            if t > 0.6 * max_iter and rating_based_entropy_value > 0.3 and len(
                ...
                feature_share_choosed_list, feature_share_abandoned_list, feature_share_random_list = new_mechanism(X,
                                                                                                                    ...
                                                                                                                    b=0.3,
                                                                                                                    part_num=2)
                # print('4 sprint choosed', feature_share_choosed_list, len(feature_share_choosed_list))
                # print('4 sprint abandoned', feature_share_abandoned_list, len(feature_share_abandoned_list))
                # print('4 sprint random', feature_share_random_list, len(feature_share_random_list))
    # Best feature subset
    Gbin = Xgb[0, :]
    ...
    # Create dictionary
    ecga_data = {'sf': Gbin, 'c': curve, 'nf': num_feat}
    # print(ecga_data)
    # print(fit)
