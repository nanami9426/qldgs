...
import numpy.matlib
import operator
from NDEDA3 import produce_unique_individuals
...
creator.create("FitnessMin2", base.Fitness, weights=(-1.0,))####minimise an objectives
creator.create("Individual2", array.array, typecode='d', fitness=creator.FitnessMin2)
toolbox = base.Toolbox()

def main(seed, xtrain, xvalid, ytrain, yvalid, opts):
    result = {'Gbin': [], 'curve': []}
    random.seed(seed)
    ...
    toolbox.register("individual", tools.initIterate, creator.Individual2, toolbox.attr_float)  ###fitness
    toolbox.register("population", tools.initRepeat, list, toolbox.individual)  ##particles
    toolbox.register("evaluate", fit_train1, data=data)
    toolbox.register("select", NDEDA2.selNS)##alg4_NSGA2
    toolbox.register("select1", NDEDA2.selection_compared_with_nearest)
    ...
    for gen in range(1, NGEN):
        ...
        if fit_num > Max_FES:
            break
    return offspring,min_fitness,unique_number,result
...
def fs(xtrain, xvalid, ytrain, yvalid, opts):
    ...
    pop,min_fitness,unique_number,result = main(seed, xtrain, xvalid, ytrain, yvalid, opts)
    ...
    NDEDA_data = {'sf': Gbin, 'c': curve, 'nf': num_feat}
