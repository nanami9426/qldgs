...
def binary_conversion(X, thres, N, dim):
    ...
    #         else:
    #             Xbin[i,d] = 0
    Xbin = (X > thres).astype(int)  # 向量化操作，生成二进制矩阵
        ...
        for i in range(N):
            ...
            if r1  < 0.5:
                ...
                if abs(A) < 1:
                    ...
                    Dx = np.abs(C * Xgb[0, :] - X[i, :])  # 计算每个维度的Dx
                    X[i, :] = Xgb[0, :] - A * Dx  # 一次性更新所有维度
                    X[i, :] = np.clip(X[i, :], lb[0, :], ub[0, :])  # 限制在边界内
                ...
                # {2} Search for prey
                    ...
                    #     # Boundary
                    #     X[i,d] = boundary(X[i,d], lb[0,d], ub[0,d])
                    # 假设 X、C、A、lb、ub 和 N 都是 numpy 数组
                    k = np.random.randint(low=0, high=N, size=dim)  # 随机选择N个whale，每个维度对应一个
                    Dx = np.abs(C * X[k, range(dim)] - X[i, range(dim)])  # 计算每个维度的 Dx
                    X[i, range(dim)] = X[k, range(dim)] - A * Dx  # 一次性更新所有维度
                    X[i, range(dim)] = np.clip(X[i, range(dim)], lb[0, range(dim)], ub[0, range(dim)])  # 限制在边界内
            
            # {3} Bubble-net attacking 
            elif r1 >= 0.5:
                # 假设 X, Xgb, b, l, lb 和 ub 都是 numpy 数组
                dist = np.abs(X[i, :] - Xgb[0, :])  # 计算所有维度的距离
                X[i, :] = dist * np.exp(b * l) * np.cos(2 * np.pi * l) + Xgb[0, :]  # 一次性更新所有维度
                X[i, :] = np.clip(X[i, :], lb[0, :], ub[0, :])  # 限制在边界内
