...
def fs(xtrain, xvalid, ytrain, yvalid, opts):
    ...
    F = 0.8 #差分进化因子
    CR = 0.9 # 交叉概率
    de_x = np.random.rand(num_intervals, dim)   # pso的位置，用于寻找特征
    de_v = np.random.rand(num_intervals, dim)  # pso的速度，用于寻找特征
    de_fitness = np.zeros(num_intervals)    # pso的适应度
    de_xbest = np.zeros((num_intervals, dim))  # pso的区间最优位置
    de_gbest = np.random.binomial(1, 0.5, size=dim)  # pso的区间最优位置
    g_best = (de_gbest >= 0.5).astype(int)
    fitGB = Fun(xtrain, xvalid, ytrain, yvalid, g_best, opts)
    de_fitbest = np.zeros(num_intervals)    # pso的最优适应度
    for i in range(num_intervals - 1, -1, -1):
        ...
        de_x[i, :] = np.random.rand(dim)
        de_xbest[i, :] = np.random.rand(dim)
        X_best = (de_xbest[i, :] >= 0.5).astype(int)
        de_fitbest[i] = Fun(xtrain, xvalid, ytrain, yvalid, X_best, opts)
        for j in range(valued_num):
            Nsfn = random.randint(intervals[i] + 1, intervals[i + 1])
            idxs = [o for o in range(num_intervals) if o != i]
            a, b, c = np.random.choice(idxs, 3, replace=False)

            mutant = de_x[a, :] + F * de_x[b, :] - de_x[c, :]
            mutant =  np.clip(mutant, 0, 1)
            trial = np.copy(de_x[i, :])
            for p in range(dim):
                if np.random.rand() < CR:
                    trial[p] = mutant[p]
            X_cur = (trial >= 0.5).astype(int)
            fitc = Fun(xtrain, xvalid, ytrain, yvalid, X_cur, opts)
            temp_x[i, j, :] = X_cur.copy()
            temp_fitness[i, j] = fitc
            if fitc < de_fitbest[i]:
                de_x[i, :] = trial.copy()
                de_fitbest[i] = fitc
            if fitc < fitGB or (fitc == fitGB and Nsfc < Nsfb):
                pso_gbest = de_x[i, :].copy()
        ...
        for i in range(num_intervals - 1, -1, -1):
            ...
            for j in range(valued_num):
                ...
                for p in range(dim):
                    if np.random.rand() < CR:
                        trial[p] = mutant[p]
                trial = prune_solution(trial, Nsfn)
                X_cur = (trial >= 0.5).astype(int)
                fitc = Fun(xtrain, xvalid, ytrain, yvalid, X_cur, opts)
                temp_x[i, j, :] = X_cur.copy()
                temp_fitness[i, j] = fitc
