...
def fs(xtrain, xvalid, ytrain, yvalid, opts):
    ...
    ga_x = np.random.rand(num_intervals, dim)   # ga的位置，用于寻找特征
    ga_v = np.random.rand(num_intervals, dim)  # pso的速度，用于寻找特征
    ga_fitness = np.zeros(num_intervals)    # pso的适应度
    ga_xbest = np.zeros((num_intervals, dim))  # pso的区间最优位置
    ga_gbest = np.random.binomial(1, 0.5, size=dim)  # pso的区间最优位置
    g_best = (ga_gbest >= 0.5).astype(int)
    fitGB = Fun(xtrain, xvalid, ytrain, yvalid, g_best, opts)
    ga_fitbest = np.zeros(num_intervals)    # pso的最优适应度
    for i in range(num_intervals - 1, -1, -1):
        ...
        _,Nsfn = init_Xc(dim, intervals[i], intervals[i + 1])
        ga_x[i, :] = np.random.rand(dim)
        ga_xbest[i, :] = np.random.rand(dim)
        X_best = (ga_xbest[i, :] >= 0.5).astype(int)
        ga_fitbest[i] = Fun(xtrain, xvalid, ytrain, yvalid, X_best, opts)
        for j in range(valued_num//2):
            Nsfn = random.randint(intervals[i] + 1, intervals[i + 1])
            elite_indices = ga_fitness.argsort()[:2]
            new_population = ga_x[elite_indices, :]

            parent1, parent2 = ga_x[np.random.choice(num_intervals, 2, replace= False)]
            ...
            child1 = prune_solution(child1, Nsfn)
            X_cur = (child1 >= 0.5).astype(int)
            fitc = Fun(xtrain, xvalid, ytrain, yvalid, X_cur, opts)
            temp_x[i, j , :] = X_cur.copy()
            temp_fitness[i, j] = fitc
            if fitc <= ga_fitbest[i]:
                ga_xbest[i, :] = child1.copy()
                ga_x[i, :] =  child1.copy()
                ga_fitbest[i] = fitc
            if fitc < fitGB or (fitc == fitGB and Nsfn < Nsfb):
                ga_gbest = ga_x[i, :].copy()
            ...
            child2 = prune_solution(child1, Nsfn)
            X_cur = (child2 >= 0.5).astype(int)
            fitc = Fun(xtrain, xvalid, ytrain, yvalid, X_cur, opts)
            temp_x[i, j + valued_num // 2, :] = X_cur.copy()
            temp_fitness[i, j + valued_num // 2] = fitc
            if fitc <= ga_fitbest[i]:
                ga_xbest[i, :] = child2.copy()
                ga_x[i, :] =  child2.copy()
                ga_fitbest[i] = fitc
            if fitc < fitGB or (fitc == fitGB and Nsfn < Nsfb):
                ga_gbest = ga_x[i, :].copy()
                Xgb = temp_x[i, j + valued_num // 2, :].copy()  # 不必要的复制，直接赋值即可
                fitGB = fitc
                Nsfb = Nsfn
                gbest_fit_cur = fitc
                gbest_len_cur = Nsfn
